...
function get_user_info($id=0)
{
    ...
    $time = date('Y-m-d');
    $sql  = 'SELECT u.user_type,u.user_id, u.email, u.user_name, u.user_money, u.pay_points'.
            ' FROM ' .$GLOBALS['hhs']->table('users'). ' AS u ' .
            " WHERE u.user_id = '$id'";
...
function get_shop_about()
{
    $sql = 'SELECT c.cat_id, c.cat_name, c.sort_order, a.article_id, a.title, a.file_url, a.open_type ' .
            ...
            'ON a.cat_id = c.cat_id WHERE c.cat_type = 2 AND a.is_open = 1 ' .
...
//     echo $app.'-------'.$cat;exit;
    switch ($app)
    {
        ...
        case 'search':
            ...
            //cxqadd
        case 'search_cars':
            	$uri_args = array('cid' => $cat, 'region_id' =>$city_id, 'suppliers_id'=>$sid,  'id'=> $id, 'search'=>$search,'price'=>$pr);
            	break;
            //cxqend
        case 'exchange':
    ...
    else
    {
        ...
        {
        	//echo $page."==".$_offset."==".$_pagenum."--".$page_count;exit();
            ...
            $pager['page_kbd']  = ($_pagenum < $page_count) ? true : false;
            $pager['page_number'] = array();
    //  echo $app.','.$uri_args.','.''.','. $i.','. $keywords;exit;
	//print_r($uri_args);
// 			print_r($_from.','.$_to);exit;
            for ($i=$_from;$i<=$_to;++$i)
            {
//             	echo build_uri($app, $uri_args, '', $i, $keywords);exit;
                $pager['page_number'][$i] = build_uri($app, $uri_args, '', $i, $keywords);
            }
          // print_r($pager['page_number']);exit;
        }
    }
    if (!empty($sch_array))
    {
...
function show_message_vip($content, $links = '', $hrefs = '', $type = 'info', $auto_redirect = true)
{
	assign_template();
	...
	$GLOBALS['smarty']->assign('hrefs', $hrefs);
	
	$GLOBALS['smarty']->assign('auto_redirect', $auto_redirect);
	$GLOBALS['smarty']->assign('message', $msg);
	$GLOBALS['smarty']->display('message_vip.dwt');
