 ...
 * @return  void
 */
function get_goods_info($goods_id,$date_at=null)
{
    $time = gmtime();
    if(!empty($date_at)){
        $sql = 'SELECT g.*, c.measure_unit, b.brand_id, b.brand_name AS goods_brand, m.type_money AS bonus_money,p.`price` as today_price, ' .
                'IFNULL(AVG(r.comment_rank), 0) AS comment_rank, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS rank_price " .
            'FROM ' . $GLOBALS['hhs']->table('goods') . ' AS g ' .
            'LEFT JOIN ' . $GLOBALS['hhs']->table('goods_price') . ' as p ON g.`goods_id` = p.`goods_id` AND p.`date_at` = "' .$date_at . '"'.
            ...
            $sql="select * from ". $GLOBALS['hhs']->table("goods")." where goods_id=".$goods_id;
        }
 ...
 * @return  array
 */
function get_goods_properties2($goods_id)
{
	/* 对属性进行重新排序和分组 */
	$sql = "SELECT attr_group ".
			"FROM " . $GLOBALS['hhs']->table('goods_type') . " AS gt, " . $GLOBALS['hhs']->table('goods') . " AS g ".
	...
	foreach ($res AS $row)
	{
		...
		if ($row['attr_type'] == 0)
		{
			$group = (isset($groups[$row['attr_group']])) ? $groups[$row['attr_group']] : 'attr';
 ...
 * @return  void
 */
function spec_price_my($spec,$days)
{
    if (!empty($spec))
    {
        if(is_array($spec))
...
function get_goods_attr($goods_id)
{
    ...
    while ($goods_attr = $GLOBALS['db']->fetchRow($res))
    {
        ...
        //$attr_list[$goods_attr['attr_id']]['goods_attr_list']['attr_price'] = $goods_attr['attr_price'];
    }
